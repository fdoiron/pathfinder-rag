...
# Note: imports _HEADING_RE from implementation so shares the same definition of heading as the code.
...
# Note: imports _HEADING_RE from implementation so shares the same definition of heading as the code.
# bug in regex would pass the test
# test_split_golden_rod_of_wonder_matches_hand_written_sections is an independent check
@pytest.mark.parametrize('slug', GOLDEN_SLUGS)
...
# Change detrctor but not a correctness guarentee as they are generated by _split_by_markdown_heading
