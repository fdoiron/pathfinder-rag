...
in-process embedder. Run directly with:
    ...
    uv run python scripts/prototype_cli.py <command>
"""
...
def embed(
    ...
    client = make_genai_client(settings)
    embedder = VertexEmbedder(client, settings)
    out = embed_corpus(df, embedder, text_columns=['title', 'body_md'])
    manifest = CorpusManifest.build(settings, out, source_file=input_file, text_columns=['title', 'body_md'])
...
def search(
    ...
    """
    Search the embedded corpus for the query and print the top-k results.
    """
